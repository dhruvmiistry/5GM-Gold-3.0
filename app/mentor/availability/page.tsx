...
export default function MentorAvailabilityPage() {
  ...
  const load = useCallback(() => {
    ...
    fetch('/api/mentor/availability')
      .then(r => r.json())
      .then(data => { setWeekly(data.weekly ?? []); setOverrides(data.overrides ?? []); setLoading(false) })
  ...
  const remove = async (type: 'weekly' | 'override', rowId: string) => {
    const res = await fetch(`/api/mentor/availability?type=${type}&rowId=${rowId}`, { method: 'DELETE' })
    if (!res.ok) { showToast('Failed to remove'); return }
    load()
  }

  if (loading) {
    return <div className="dashboard-bg min-h-[calc(100vh-3rem)] flex items-center justify-center"><Loader2 size={20} className="animate-spin text-[#c9a84c]" /></div>
  }
    ...
    <div className="dashboard-bg min-h-[calc(100vh-3rem)]">
      <div className="max-w-3xl mx-auto px-6 md:px-8 py-8 space-y-8">
        <div>
          ...
          <h1 className="text-2xl font-light text-white tracking-tight">Availability</h1>
          <p className="text-[#5a5a66] text-sm mt-1">Weekly hours are interpreted in your configured timezone (set by an admin).</p>
        </div>
