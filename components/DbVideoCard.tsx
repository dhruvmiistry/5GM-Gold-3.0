...
function getMuxThumbnail(playbackId: string | null) {
  if (!playbackId) return null
  return `https://image.mux.com/${playbackId}/thumbnail.jpg?time=2&width=640`
}
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-video bg-[#0d0d0f] overflow-hidden">
        ...
        {/* Waveform fallback (shown when no thumbnail) */}
        {!thumbnail && (
      ...
      <div className="p-5">
        <div className="flex items-center gap-2 mb-3">
          ...
          {video.release_date && (
            <span className="text-[#5a5a66] text-[11px]">{formatReleaseDate(video.release_date)}</span>
          )}
        </div>
        ...
        {video.description && (
          <p className="text-[#5a5a66] text-xs leading-relaxed mb-4 line-clamp-2">{video.description}</p>
        )}
        ...
        {video.analyst_name && (
          <div className="flex items-center gap-2 pt-3 border-t border-[rgba(255,255,255,0.05)]">
            ...
            <span className="text-[#5a5a66] text-[11px]">{video.analyst_name}</span>
          </div>
        )}
      </div>
    </motion.div>
